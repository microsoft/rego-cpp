...
env_ok(pattern, "string", value) if {
	pattern == value
}

env_ok(pattern, "re2", value) if {
...
idName_ok(pattern, "any", value) := true
...
largest_caps_sets_for_all(containers, privileged) := largest_caps_sets if {
	...
	counts := [num_caps |
		caps := filtered[_]
		num_caps := (((count(caps.bounding) + count(caps.effective)) + count(caps.inheritable)) + count(caps.permitted)) + count(caps.ambient)
	]
	max_count := max(counts)
...
create_container := {
	"metadata": [updateMatches, addStarted],
	"env_list": env_list,
	"caps_list": caps_list,
	...
	addStarted := {
		...
		"action": "add",
		"key": input.containerID,
		"value": {"privileged": input.privileged},
	}
}
...
enforcement_point_info := {"available": false, "default_results": {"allow": false}, "unknown": false, "invalid": false, "version_missing": true} if {
	policy_api_version == null
}

enforcement_point_info := {"available": available, "default_results": default_results, "unknown": false, "invalid": false, "version_missing": false} if {
...
enforcement_point_info := {"available": false, "default_results": {"allow": false}, "unknown": false, "invalid": true, "version_missing": false} if {
	enforcement_point := data.api.enforcement_points[input.name]
	semver.compare(data.api.version, enforcement_point.introducedVersion) < 0
}
...
extract_fragment_includes(includes) := fragment if {
	...
	fragment := {include: 
	objects[include] | include := includes[_]}
}
