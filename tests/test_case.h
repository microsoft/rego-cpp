...
namespace rego_test
{
  ...
  private:
    ...
    static std::vector<std::string> get_modules(const std::filesystem::path& dir, const Node& mapping);
    static void diff(
      const std::string& actual, const std::string& wanted, std::ostream& os);
    static std::optional<TestCase> create_from_node(const std::filesystem::path& filename, const Node& test_case_map);
