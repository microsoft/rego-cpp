...
namespace
{
  ...
  Node set_or_query(Node brace, bool has_commas)
  {
    ...
    if (before != nullptr && op_tokens.contains(before->type()))
    {
      if (has_commas)
      {
    ...
    else if (after != nullptr && op_tokens.contains(after->type()))
    {
      if (has_commas)
      {
        return Set << comma_separation(group);
  ...
  PassDef prep()
  {
    return {
      ...
      dir::topdown | dir::once,
      {
        ...
        In(Module) * T(Group)
            << (T(Package) * (T(Var, Dot, Square) / Keyword)++[Group] * End) >>
        ...
        In(Module) *
            (T(Group)[Group]
             << (T(Import) * (T(Var, Dot, Square) / Keyword)++ * End)) >>
          [](Match& _) {
            ...
            Node refgroup = NodeDef::create(RefGroup);
            size_t refsize = _(Group)->size();
            if (refsize > 2 && _(Group)->at(refsize - 2) == As)
            {
              refsize -= 2;
            }

            NodeRange path(_(Group)->begin() + 1, _(Group)->begin() + refsize);
            return Import << (RefGroup << path) << alias->clone();
          },

        In(Group) * (T(Dot)[Dot] * Keyword[Var]) >>
          [](Match& _) { return Seq << _(Dot) << (Var ^ _(Var)); },

        In(Group) * (Keyword[Var] * T(Dot)[Dot]) >>
          [](Match& _) { return Seq << (Var ^ _(Var)) << _(Dot); },
  ...
  const auto wf_ref_args_tokens = (wf_prep_tokens | RefArgDot | RefArgBrack);
  ...
  PassDef ref_args()
  {
    return {
      ...
      dir::bottomup,
      {
        In(Group, RefGroup) * (T(Dot) * T(Var)[Var]) >>
  ...
  PassDef groups()
  {
      ...
      dir::bottomup | dir::once,
      {
        ...
        T(Brace)[Brace] << T(Group)[Group] >>
          [comma_groups, colon_groups, or_groups](Match& _) {
            ...
            else if (colon_groups->contains(group))
            {
              colon_groups->erase(group);
              if (or_groups->contains(group))
              {
            ...
            else if (or_groups->contains(group))
            {
              or_groups->erase(group);
              auto pos = _(Group)->find_first(Or, _(Group)->begin());
              Nodes item(_(Group)->begin(), pos);
        ...
        T(Square) << T(Group)[Group] >>
          [comma_groups, or_groups](Match& _) {
            ...
            else if (comma_groups->contains(group))
            {
              return Array << comma_separation(_(Group));
            }
            else
    ...
    pass.pre(Or, [or_groups](Node node) {
      ...
      {
        Node n = *it;
        if (n == Comma || (n != Colon && op_tokens.contains(n->type())))
        {
          return 0;
        }
      }
    ...
    PassDef pass = {
      ...
      dir::bottomup | dir::once,
      {
        ...
        In(Group) * (T(Else) * ~T(If) * T(Query)[Query]) >>
          [](Match& _) {
            return Else << (Expr << (Term << (Scalar << (True ^ "true"))))
                        << _(Query);
          },
      ...
      dir::bottomup | dir::once,
      {
        ...
        In(Group) *
          ...
          [](Match& _) {
            ...
            Node rulehead = RuleHead
              << (RuleRef << ruleref)
              << (RuleHeadObj
                  << value << (Expr << (Term << (Scalar << (True ^ "true")))));

            return Rule << (False ^ "false") << rulehead
                        << (RuleBodySeq << _[RuleBodySeq]);
          },
        ...
        In(Group) *
            (~T(Default)[Default] *
             ...
             ~T(If)[If] * T(Query, Else)++[RuleBodySeq] * T(NewLine)) >>
          [](Match& _) {
            return Rule << (_(Default) ? (True ^ "true") : (False ^ "false"))
  ...
  const auto wf_literals_tokens = (wf_rules_tokens | Literal) -
    (Expr | NotExpr | SomeDecl | With | NewLine | Version);
  ...
  PassDef literals()
  {
    return {
      ...
      dir::bottomup | dir::once,
      {
        ...
        T(NewLine) >> [](Match&) -> Node { return nullptr; },
  ...
  PassDef structure()
  {
    return {
      ...
      dir::bottomup | dir::once,
      {
        ...
        In(Module) * (T(Group, Policy) * T(Import, Version)[Import]) >>
          [](Match& _) {
            return err(
              _(Import),
        ...
        In(Module) *
            (T(Package) * T(Import, Version)++ * T(Package)[Package]) >>
          [](Match& _) {
            return err(
              _(Package),
              "Only one package statement allowed per module",
              WellFormedError);
          },
        ...
        T(Module)[Module] << End >>
          [](Match& _) {
            return err(
              _(Module), "Missing a package statement", WellFormedError);
          },
      }};
  }
    ...
    return {
      ...
      dir::bottomup | dir::once,
      {
        In(Top) * (T(Query) << (T(Literal)[Literal] * End)) >>
          [](Match& _) { return _(Literal)->front(); },

        T(Expr) << (T(Term)[Term] << T(Object, Array, Set, Scalar)) >>
          [](Match& _) { return _(Term); },

        T(String) << T(JSONString)[JSONString] >>
          [](Match& _) {
            return JSONString ^
              (strip_quotes(_(JSONString)->location().view()));
          },

        T(String) << T(RawString)[RawString] >>
          [](Match& _) {
