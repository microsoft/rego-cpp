...
namespace {
  ...
  void collect_refheads(const Node& node, RefHeads refheads)
  {
    if(node->type() == Rule){
      ...
      refheads->insert(flatten_ref(concat_refs(prefix_ref, ref)));
    }else{
      for(auto& child : *node){
  ...
  PassDef lift_refheads()
  {
    ...
    PassDef lift_refheads = {
      In(Policy) * T(Rule)[Rule]([](auto& n){
        Node rule = *n.first;
        return (rule / RuleHead / RuleRef)->front()->type() == Ref;
      }) >> [refheads](Match& _){
        ...
        return Lift << ModuleSeq << (Module << (Package << new_package_ref) << imports << (Policy << _(Rule)));
      },
    };
