...
namespace
{
  ...
  BuiltIn concat_factory()
  {
    const Node concat_decl = bi::Decl
      << (bi::ArgSeq << (bi::Arg
                         << (bi::Name ^ "x")
                         << (bi::Description ^ "the first array")
                         << (bi::Type
      ...
      << (bi::Result << (bi::Name ^ "z")
                     ...
                     << (bi::Type
                         << (bi::DynamicArray << (bi::Type << bi::Any))));
    return BuiltInDef::create({"array.concat"}, concat_decl, concat);
  }
  ...
  BuiltIn reverse_factory()
  {
    ...
    return BuiltInDef::create({"array.reverse"}, reverse_decl, reverse);
  }
...
namespace rego
{
  namespace builtins
  {
    ...
    {
      assert(name.view().starts_with("array."));
      std::string_view view = name.view().substr(6); // skip "array."
      if (view == "concat")
      {
