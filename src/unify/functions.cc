...
namespace
{
  ...
  const char* compr_name(Node node)
  {
    if (node == ArrayCompr)
    {
      return "array-compr";
    ...
    throw std::runtime_error("Invalid comprehension type");
  }
  ...
  PassDef functions()
  {
    PassDef functions{
      ...
      dir::topdown,
      {
        ...
        In(UnifyExpr, ArgSeq) *
            (T(Membership)
             << ((T(ExprSeq) << (T(Expr)[Item] * End)) * T(Expr)[ItemSeq])) >>
          [](Match& _) {
            ACTION();
            return Function
        ...
        In(UnifyExpr, ArgSeq) * (T(ArrayCompr, SetCompr, ObjectCompr))[Compr] >>
          [](Match& _) {
            ACTION();
            std::string name = compr_name(_(Compr));
            std::transform(
        ...
        In(UnifyExpr, ArgSeq) *
            (T(Term) << (T(ArrayCompr, SetCompr, ObjectCompr)))[Compr] >>
          [](Match& _) {
            ACTION();
            std::string name = compr_name(_(Compr));
            std::transform(
        ...
        In(UnifyExpr, ArgSeq) * (T(UnaryExpr) << T(Expr)[Expr]) >>
          [](Match& _) {
            ACTION();
            return Function << (JSONString ^ "unary")
                            << (ArgSeq << unwrap_node(_(Expr)->front()));
          },

        In(UnifyExpr, ArgSeq) *
            (T(ArithInfix) << (T(Expr)[Lhs] * Any[Op] * T(Expr)[Rhs])) >>
          [](Match& _) {
            ACTION();
        ...
        In(UnifyExpr, ArgSeq) *
            (T(BinInfix) << (T(Expr)[Lhs] * Any[Op] * T(Expr)[Rhs])) >>
          [](Match& _) {
            ACTION();
        ...
        In(UnifyExpr, ArgSeq) *
            (T(BoolInfix) << (T(Expr)[Lhs] * Any[Op] * T(Expr)[Rhs])) >>
          [](Match& _) {
            ACTION();
        ...
        In(UnifyExpr, ArgSeq) *
            (T(ExprCall) << (T(Var)[Var] * T(ExprSeq)[ExprSeq])) >>
          [](Match& _) {
            ACTION();
            Node argseq = ArgSeq << _(Var);
            for (auto& child : *_(ExprSeq))
            {
              argseq << unwrap_node(child);
            }
        ...
        In(RuleComp, RuleFunc, RuleObj, RuleSet) * T(DataTerm)[DataTerm] >>
          [](Match& _) {
            ACTION();
            return Term << convert_data(_(DataTerm));
          },
