      ...
      if (is_constant(key))
      {
        ...
        else
        {
          ...
          boolinfixes.push_back(
            BoolInfix << (Expr << lhsval) << Equals << (Expr << rhsval));
        }
      }
      else
      {
  ...
  PassDef rulebody()
  {
    PassDef rulebody{
      ...
      dir::topdown,
      {
              ...
              else
              {
                ...
                seq << (Local << (Var ^ unify) << Undefined)
                    << (UnifyExpr
                        << (Var ^ unify)
                        << (Expr
                            << (BoolInfix << (Expr << lhs_term) << Equals
                                          << (Expr << rhs_term))));
              }
            }
        ...
        // <Expr>.<setcompr>
        In(Expr) * T(SetCompr)[SetCompr] * In(UnifyBody)++ >>
          [](Match& _) {
            ACTION();
        ...
        In(Expr) * T(AssignInfix)[Arg] >>
          [](Match& _) {
            ACTION();
            return err(_(Arg), "Invalid boolean argument");
          },

        In(RefArgBrack) * T(AssignInfix)[Arg] >>
          [](Match& _) {
            ACTION();
