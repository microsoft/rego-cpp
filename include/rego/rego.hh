...
namespace rego
{
  ...
  /// @brief Attempts to extract the value of a node as an 64-bit floating point
  /// number.
  /// @param node The node to extract from.
  ...
  /// @note If this is a synthetic node (i.e., one generated by the compiler)
  /// then this may be an empty string.
  /// @param node
  ...
  typedef BuiltIn (*BuiltInFactory)();

  /// @brief Manages the set of builtins used by an interpreter to resolve
  /// built-in calls.
  /// @details This object provides all lookup for builtins. Users can add their
  /// own custom builtins using `BuiltInsDef::register_builtin`. Built-ins from
  /// the [Rego standard
  /// library](https://www.openpolicyagent.org/docs/policy-reference/builtins/).)
  /// will be loaded as needed. Control over what parts of the standard library
  /// are available to policies is available via the `BuiltInsDef::whitelist`
  /// and `BuiltInsDef::blacklist` methods.
  class BuiltInsDef
  {
  public:
    /// @brief Constructor.
    ...
    /// @brief This method no longer needs to be called.
    /// @deprecated
    /// @return A reference to this instance.
    BuiltInsDef& register_standard_builtins();

    /// @brief Sets a whitelist of built-in names that are allowed to be loaded
    /// @details Only built-ins whose names are in this list will be allowed.
    /// Any previous list and behavior will be overridden.
    /// @param allowed the allowed built-in names
    /// @return A reference to this instance
    template <typename T>
    BuiltInsDef& whitelist(const std::initializer_list<T>& allowed)
    {
      m_lookup_behavior = BuiltInsDef::LookupBehavior::Whitelist;
      m_list.clear();
      m_list.insert(allowed.begin(), allowed.end());
      m_builtins.clear();
      return *this;
    }
    ...
    /// @details Only built-ins whose names are in this list will be allowed.
    /// Any previous list and behavior will be overridden.
    /// @param begin iterator pointing to the beginning of a range of allowed
    /// names
    /// @param end iterator pointing to the end of a range of allowed names
    /// @return A reference to this instance
    template <typename T>
    BuiltInsDef& whitelist(T begin, T end)
    {
      m_lookup_behavior = BuiltInsDef::LookupBehavior::Whitelist;
      m_list.clear();
      m_list.insert(begin, end);
      m_builtins.clear();
      return *this;
    }

    /// @brief Sets a blacklist of built-in names that are forbidden from being
    /// loaded
    /// @details Built-ins whose names are in this list will be forbidden. Any
    /// previous list and behavior will be overridden.
    /// @param forbidden the forbidden built-in names
    /// @return A reference to this instance
    template <typename T>
    BuiltInsDef& blacklist(const std::initializer_list<T>& forbidden)
    {
      m_lookup_behavior = BuiltInsDef::LookupBehavior::Blacklist;
      m_list.clear();
      m_list.insert(forbidden.begin(), forbidden.end());
      m_builtins.clear();
      return *this;
    }
    ...
    /// @details Built-ins whose names are in this list will be forbidden. Any
    /// previous list and behavior will be overridden.
    /// @param begin iterator pointing to the beginning of a range of forbidden
    /// names
    /// @param end iterator pointing to the end of a range of forbidden names
    /// @return A reference to this instance
    template <typename T>
    ...
    /// @brief Allow all built-ins to be loaded from the standard library.
    /// @return A reference to this instance
  ...
  private:
    static BuiltIn lookup(const Location& name);

    enum class LookupBehavior
    {
      AllowAll,
      Whitelist,
      Blacklist
    ...
    LookupBehavior m_lookup_behavior;
    std::set<Location> m_list;
  ...
  public:
    ...
    /// @brief Gets the maximum number of statements that will execute before a
    /// timeout.
    size_t stmt_limit() const;

    /// @brief Sets the maximum number of statements that will execute before a
    /// timeout.
  ...
  /// one or more results. The fact that a query can result in one or more
  /// results may seem surprising. The query `a = [1, 2, 3, 4]; x = a[_]; x % 2
  /// == 0; x / 2` results in two distinct solutions:
  /// ```json
  /// {"expressions":[true, true, true, 1], "bindings":{"a":[1,2,3,4], "x":2}}
  /// {"expressions":[true, true, true, 2], "bindings":{"a":[1,2,3,4], "x":4}}
  /// ```
  /// Each result consists of a list of expressions (one for each statement in
  /// the query) and a lookup table of bindings, with assigned values for each
  /// named local. This class provides methods for accessing these value. Using
  /// the output above:
  /// ```cpp
  /// rego::Node expressions0 = output.expressions();
  /// rego::Node expressions1 = output.expressions_at(1);
  /// rego::Node last0 = expressions0->back();
  /// std::cout << rego::to_key(last0);
  /// // 1
  /// rego::Node last1 = expressions1->back();
  /// std::cout << rego::to_key(last1);
  /// // 2
  /// Node a = output.binding("a");
  /// std::cout << rego::to_key(a);
  /// // [1,2,3,4]
  /// auto x0 = rego::get_int(output.binding("x"));
  /// std::cout << x0;
  /// // 2
  /// auto x1 = rego::get_int(output.binding_at(1, "x"));
  /// std::cout << x1;
  /// // 4
  /// ```
  class Output
  ...
  public:
    ...
    /// @brief Whether the Output is OK (*i.e.*, has not resulted in one or more
    /// errors)
    /// @return True if the Output is OK, otherwise false.
    bool ok() const;

    /// @brief Gets the underlying node.
    /// @return The underlying node.
    Node node() const;

    /// @brief Gets the number of results.
    /// @details There will be one result per solution to the query.
    /// @return The number of results.
    size_t size() const;

    /// @brief Gets the expressions for the result at the specified index.
    /// @param index The index of the result.
    /// @return The expressions for the result.
    Node expressions_at(size_t index) const;

    /// @brief Gets the expressions for the first result.
    /// @return The expressions for the first result.
    Node expressions() const;

    /// @brief Gets the binding for the specified name at the specified result
    /// index.
    /// @param index The index of the result.
    /// @param name The name of the binding.
    /// @return The binding for the specified name.
    Node binding_at(size_t index, const std::string& name) const;

    /// @brief Gets the binding for the specified name at the first result.
    /// @param name The name of the binding.
    /// @return The binding for the specified name.
    Node binding(const std::string& name) const;

    /// @brief Gets the JSON representation of the output.
    /// @return The JSON representation of the output.
    const std::string& json() const;

    /// @brief Gets the errors in the output.
    /// @details
    /// If the output is not OK, this method can be used to retrieve the
    /// error messages.
    /// @return The errors in the output.
  ...
  public:
    ...
    /// @brief Executes a query against the interpreter.
    /// @details
    /// This method calls Interpreter::query_node and then wraps it in an Output
    /// object.
    /// @return The result of the query.
